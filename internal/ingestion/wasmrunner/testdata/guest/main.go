...
// wasmrunner's own tests - built via `GOOS=wasip1 GOARCH=wasm go build
// -buildmode=c-shared` (a WASI reactor/library, callable exports via
// //go:wasmexport, no TinyGo needed for the HOST-side runner's own
// tests). The real SDK (sdk/connector/wasmguest) targets TinyGo, per
// the design's own explicit choice (§3.1) - this fixture exists purely
// to exercise wasmrunner's host-function wiring end-to-end without
// requiring TinyGo installed in this environment.
package main
...
func jengineFetch(configPtr, configLen uint32) int32 {
	...
	if string(cfg) == "infinite-loop" {
		...
		// pure guest-local computation - host function calls are where
		// wazero's compiled code actually checks for the module having
		// been closed. A truly empty `for {}` with zero host/memory
		// interaction is a sharper, real limitation this package's own
		// docs call out separately: wazero's Close() does not reliably
		// interrupt that shape at all (found via direct testing, not
		// theorized) - this fixture models the realistic "runaway
		// connector that's still doing something" case instead, which
		// IS interruptible.
		for {
			log(0, "looping")
		}
	...
	// Normal path: emit a couple of fixed records.

...
func (r *TransactionRepo) Create(ctx context.Context, tenantID uuid.UUID, t domain.Transaction) (domain.Transaction, error) {
	...
	err = tx.QueryRow(ctx,
		`INSERT INTO transactions (id, tenant_id, account_id, statement_id, external_ref, amount, currency, fx_rate_to_base, base_amount, value_date, booking_date, description, counterparty_ref, transaction_type, side, source_mode, ingestion_idempotency_key, status, raw_payload)
		 ...
		 RETURNING id, tenant_id, account_id, statement_id, external_ref, amount, currency, fx_rate_to_base, base_amount, value_date, booking_date, description, counterparty_ref, transaction_type, side, source_mode, ingestion_idempotency_key, status, raw_payload, created_at, updated_at`,
...
func (r *TransactionRepo) GetByID(ctx context.Context, tenantID uuid.UUID, id uuid.UUID) (domain.Transaction, error) {
	...
	err = tx.QueryRow(ctx,
		`SELECT id, tenant_id, account_id, statement_id, external_ref, amount, currency, fx_rate_to_base, base_amount, value_date, booking_date, description, counterparty_ref, transaction_type, side, source_mode, ingestion_idempotency_key, status, raw_payload, created_at, updated_at

...
- No HMAC signing, no retry/backoff, no DLQ for this path — an SSE client that misses an event due to a dropped connection re-syncs via the frontend's REST-based fallback poll (`task/frontend/10`), it does not need guaranteed delivery semantics the way outbound webhooks do.

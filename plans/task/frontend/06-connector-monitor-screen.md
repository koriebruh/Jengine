...
plans/docs/14-dashboard-frontend.md §14.4 describes MVP's frontend connector screen minimally as a "basic connector status page," with the fuller quarantine/DLQ/redrive feature set implied for the Phase 1 (V1) "full screen set." However, the master frontend task list (`plans/task/README.md`) places this task (frontend 06) in the MVP phase, and this task's own brief explicitly asks for quarantine viewer + DLQ browser + redrive — following flow 15.5. Resolve this by building **both** pieces, but treating them as two independently-shippable panels with different confidence levels:
